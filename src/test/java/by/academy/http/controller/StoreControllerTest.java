...
class StoreControllerTest {
    ...
    void findAll() throws Exception {
        mockMvc.perform(get(API_STORES)
                        .with(user(TEST_USERNAME).authorities(Role.ADMIN)))
                .andExpect(status().is2xxSuccessful())
                .andExpect(view().name(STORE_STORES))
    ...
    void create() throws Exception {
        ...
        mockMvc.perform(post(API_STORES + ADD_STORE)
                        .param(PAGE, TEST_PAGE_VALUE)
                        .param(SIZE, TEST_SIZE_VALUE)
                        .flashAttr(STORE_DTO, storeDTO)
                        .with(user(TEST_USERNAME).authorities(Role.ADMIN)))
                .andExpectAll(
                        status().is3xxRedirection(),
                        redirectedUrlPattern(API_STORES + PATTERN_PATH)
                );
    }
    ...
    void update() throws Exception {
        ...
        mockMvc.perform(post(API_STORES + ID_UPDATE, storeId)
                        .param(PAGE, TEST_PAGE_VALUE)
                        .param(SIZE, TEST_SIZE_VALUE)
                        .flashAttr(STORE_DTO, storeDTO)
                        .with(user(TEST_USERNAME).authorities(Role.ADMIN)))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrlPattern(API_STORES + PATTERN_PATH));
    }

    @Test
    void delete() throws Exception {
        Long storeId = 1L;
        String referer = API_STORES;
        mockMvc.perform(post(API_STORES + ID_DELETE, storeId)
    ...
    void goToEditPage() throws Exception {
        ...
        mockMvc.perform(get(API_STORES + EDIT_STORE_ID, storeId)
                        .param(PAGE, TEST_PAGE_VALUE)
                        .param(SIZE, TEST_SIZE_VALUE)
                        .param(SEARCH, SEARCH_TEST_VALUE)
                        .with(user(TEST_USERNAME).authorities(Role.ADMIN)))
                .andExpect(status().isOk())
                .andExpect(view().name(STORE_EDIT_STORE))
    ...
    void goToAddPage() throws Exception {
        mockMvc.perform(get(API_STORES + ADD_STORE)
                        .param(PAGE, TEST_PAGE_VALUE)
                        .param(SIZE, TEST_SIZE_VALUE)
                        .param(SEARCH, SEARCH_TEST_VALUE)
                        .with(user(TEST_USERNAME).authorities(Role.ADMIN)))
                .andExpect(status().isOk())
                .andExpect(view().name(STORE_ADD_STORE))
                .andExpect(model().attribute(PAGE, 0))
                .andExpect(model().attribute(SIZE, 10))
                .andExpect(model().attribute(SEARCH, SEARCH_TEST_VALUE));
    }
